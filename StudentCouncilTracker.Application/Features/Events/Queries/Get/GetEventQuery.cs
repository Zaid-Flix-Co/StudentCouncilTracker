...
public class GetEventQueryHandler(IEventRepository catalogEventRepository) : IRequestHandler<GetEventQuery, IOperationResult<ListDto>>
{
    public async Task<IOperationResult<ListDto>> Handle(GetEventQuery request, CancellationToken cancellationToken)
    {
        var result = OperationResult.CreateResult(await catalogEventRepository.GetList(request.Filter));
        return result;
    }
}