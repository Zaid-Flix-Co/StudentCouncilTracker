...
namespace StudentCouncilTracker.Desktop.Admin.Services.Catalogs.Events;
...
public class EventService(IHttpClientFactory clientFactory, IUserProvider userProvider) : BaseCatalogService(clientFactory, userProvider)
{
    ...
    public async Task<OperationResult<EventDtoJournal>> GetJournalAsync(bool isActive)
    {
        var result = await SendAsync<EventDtoJournal>("GetJournal", HttpMethod.Post, isActive);
        return result;
    }
